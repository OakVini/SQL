...
select NRVOO, DTSAIDA from t_voo
...
select NMCLI from T_CLIENTE c join T_PAIS p on c.CDPAIS_CLI = p.CDPAIS
where p.NMPAIS = 'Brasil';

select NMCLI from T_CLIENTE c, T_PAIS p
where p.CDPAIS = c.CDPAIS_CLI and p.NMPAIS = 'Brasil';
...
select CDEQU from dbo.T_EQUIPAMENTO
where IDTIP_PROP = 'M' and DCTIP_EQP not in ('Jato')
...
select NMCIA from dbo.T_COMPANHIA
...
-- dias.
select * from T_VOO
where DTSAIDA between '1993-01-01' and '1993-01-10'
